...
var ControllerUnit = React.createClass({
  ...
  render: function () {
    ...
    if (this.props.arrange.isCenter) {
      controlelrUnitClassName += " is-center";/*增加居中态的样式*/
  ...
  */
  rearrange: function (centerIndex) {
    var imgsArrangeArr = this.state.imgsArrangeArr,
      ...
      imgsArrangeTopArr = [],
      topImgNum = Math.floor(Math.random() * 2),    // 取一个或者不取0-2,取一个或者不取
      topImgSpliceIndex = 0,

      imgsArrangeCenterArr = imgsArrangeArr.splice(centerIndex, 1);//返回的是centerIndex位置的信息，并且可以在原数组中删除
